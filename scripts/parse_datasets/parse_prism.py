...
def build_pointing_dataset(split: str, num_proc: int = 10) -> datasets.Dataset:
    ...
    file_loc_map = {}
    paths = ["/DATA/disk0/data/PRISM/PRISM-train"]
    tar_files = []
    for path in paths:
        path = str(path)
...
if __name__ == "__main__":
    dataset = build_pointing_dataset("train")
    save_path = "/DATA/disk1/data/PRISM/train"
    ...
    with open(os.path.join(os.path.dirname(save_path), "conversations.json"), "w") as f:
